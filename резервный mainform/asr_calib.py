...
def main1(com, slave_adress, ip_adress, port, ):
    message = ""
    ADC_IN1 = []
    ADC_IN2 = []
    ADC_IN3 = []
    ...
    ADC_OUT = []
    try:
        ser = Modbus(com, 115200,  slave_adress)
        bp = PowerSupply(ip_adress, port)


        message = message + "Соединение\n"
        assert bp.connection()
        message = message + "!Включить удаленный доступ\n"
        assert bp.remote("ON")
        message = message + "!Установить напряжение\n"
        assert bp.set_voltage(220)
